...
Other_cost=ones(1,20)*WT_Rated_power*30; %其他成本 元/10^6kW
Load_rate=0.05; %偿债利率
...
z_true = P_wind_sum/( 1/(1000/1000)/(Ele_effi(BAUorOTL,t/5+1)*0.0252) + 0.84)*9.8181; % z为 氨厂全生命周期之和(单位转换为10^10kg=1董^_^)的倒数
...
% 
C = [C,P_Ele_z>=Ele_lowbound(BAUorOTL,t/5+1)*index_Inst_z_Ele];%电解槽出力*z范围为0.2-1
...
H_AA_z = A_AA_z/(1000/1000); %制氨厂耗氢速率*z 10^6kg
P_AA_z = A_AA_z*0.84; %制氨厂耗电功率*z 10^6kW [http://tjj.nanjing.gov.cn/material/njnj_2019/gongye/7-13.htm]
...
M_Inst_Capa_AA_z = 100*max(P_wind_ty)/(0.84); %一个恒大于制氨厂容量*z的极大值
...
C = [C,P_AA_z>=0*index_Inst_z_AA]; %制氨速率范围为0.3-1装机容量
C = [C,P_AA_z<=1*index_Inst_z_AA];

for i = 2:time_long
    C = [C, P_AA_z(i)-P_AA_z(i-1)<=Inst_Capa_AA_z*1];
    C = [C, P_AA_z(i)-P_AA_z(i-1)>=-Inst_Capa_AA_z*1];
end
...
% 氨厂成本计算
Cost_offshore_AA=0; %海上AA投资成本
Total_AA_Plant_z=Inst_Capa_AA_z*Cost_offshore_AA; %当前网格所需AA成本（千元）
...
Total_B_z = 0.35;
%低温储氨容器成本
Total_AS_z = A_AA_z_sum_season_max * 20.2*10^6*6.76/25000/1000*R(3,t/5+1)*2;
%水处理
...
Onshore_AA_z = A_AA_z_sum_season_max/(8760/4) * 4709.09 * 6.76*R(1,t/5+1);%陆上AA投资成本

%优化目标
obj = (Total_BT_z + Total_HE_z + Total_AA_z + Total_Plant*z +Total_Ele_z + Total_intrayarray*z  +Total_AS_z +Total_water+Total_hycomp + Onshore_AA_z)/z_true/(1000/176.5); %制氨LOCA
...
Onshore_AA = value(Onshore_AA_z)/z/10000;
...
Lcoa_onshore_AA = value(Onshore_AA)/z_true/(1000/176.5);

LCOH=Loca_wind+Loca_Ele+Loca_BT+Loca_HE+Lcoa_water+Lcoa_hycomp;
...
Result = [wangge,ty,tx,WT_Rated_power,Inst_Capa_Ele,Inst_Capa_BT,Inst_Capa_HE,Inst_Capa_AA,Total_power,Total_hour_use,Total_H,Total_H_year,Total_A,Total_A_year,...
    Total_Plant/10000,Total_Ele,0,Total_AA,0,Total_BT,Total_HE,Total_B,Total_AS,...
    Loca_wind,  Loca_Ele, 0 , Loca_AA, 0, Loca_BT, Loca_HE, Loca_B,Loca_AS,Lcoa_water,Lcoa_hycomp,...
    LOCA,LCOH,mip_gap,z];
